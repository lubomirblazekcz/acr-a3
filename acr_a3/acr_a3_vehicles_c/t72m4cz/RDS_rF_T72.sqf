uiNameSpace setVariable ["ACR_A3_t72_Ctrl",_this select 0];
...
[] spawn {
	...
	_c=uiNamespace getVariable "ACR_A3_t72_Ctrl" displayCtrl 158;
	while{player==gunner _v OR player==driver _v}do
	{
		_f=round (call ACR_A3_fnc_getFOV*_k);
		waitUntil{sleep 0.05;_f!=(round (call ACR_A3_fnc_getFOV*_k))};
		if(player==gunner _v)then{
			if(round (call ACR_A3_fnc_getFOV*_k)==9 AND _v animationPhase "PS_SR_hide" !=0)then
			{
				{_v animate [_x,0]}foreach ["PS_SR_hide", "PS_LR_hide", "PS_R_hide", "PS_E_hide"];_c ctrlSetText "\acr_a3\acr_a3_vehicles\t72m4cz\data\TPD_K1_lasermark_6x.paa";
			}else{
				if(_v animationPhase "PS_SR_hide" ==0 AND round (call ACR_A3_fnc_getFOV*_k)!=9)then{{_v animate [_x,0.6]}foreach ["PS_SR_hide", "PS_LR_hide", "PS_R_hide", "PS_E_hide"];_c ctrlSetText "";};
			};
		};
		sleep 0.1;
	};
		...
		if(player != driver _v)then{
			...
			//komputer balistyczny
				...
				if(_t>400 AND _t<3400-(990*(fog+rain)/2) )then{
					[_t] call ACR_A3_fnc_rF_T72_Animate
				}else{if(cameraView=="gunner")then{playSound "ACR_A3_RDS_Error"};sleep 2};
			}else{
				//ręczna korekta
